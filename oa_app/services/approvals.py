...
def find_matching_open_request(
    ...
    *,
    ...
    """Return an existing open request with the exact same payload, if any."""
...
def get_request(
    ...
    *,
    ...
    """Get one request by id (preferred) or by legacy sheet row number."""
    if _use_db():
        if not req_id:
            return None
        sb = get_supabase()

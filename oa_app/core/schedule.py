...
from ..config import DAY_CACHE_TTL_SEC, HEADER_MAX_COLS, OA_PREFIX
from .models import SheetInfo, TimeBlock
from .quotas import (
    first_row,
    invalidate_day_cache,
    read_cols_exact,
    read_day_column_map_cached,
    read_col_band_map_cached,
)
from .utils import canon_name_like_cell, fmt_time, is_time_token, matches_exact_oa_label, normalize_day, time_slots
...
class Schedule:
    ...
    def add_shift(self, info: SheetInfo, col: int, blocks: List[TimeBlock], name: str) -> None:
        ...
        # Tight band read (only what's needed for the lane search) for speed.
        lane_count = self._lane_count_for_blocks(blocks)
        rows_needed = [b.lane_rows[i] for i in range(lane_count) for b in blocks] if lane_count else []
        if rows_needed:
            band_min, band_max = min(rows_needed), max(rows_needed)
            day_col_map = read_col_band_map_cached(info, col, band_min, band_max, ttl_sec=DAY_CACHE_TTL_SEC)
        else:
